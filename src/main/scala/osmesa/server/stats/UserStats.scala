...
object UserStats {
  ...
  def _byId(id: Long)(implicit xa: Transactor[IO]): IO[Either[OsmStatError, UserStats]] =
    (_selectF ++ fr"WHERE id = $id")
      .query[UserStats]
      .option
      .transact(xa)
      .map {
  ...
  def _getPage(pageNum: Int, pageSize: Int = 25)(implicit xa: Transactor[IO]): IO[ResultPage[UserStats]] = {
    val offset = pageNum * pageSize + 1
    (_selectF ++ fr"ORDER BY id ASC LIMIT $pageSize OFFSET $offset")
      .query[UserStats]
      .to[List]
      .map({ ResultPage(_, pageNum) })
      .transact(xa)
